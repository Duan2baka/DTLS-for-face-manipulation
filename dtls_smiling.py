    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for j in range(data.size(1)):
                ...
                overall_dis_real = overall_dis_real + self.BCE_loss(score_true, GAN_true) * float(label[j][0])
            ...
            for j in range(len(x_recon)):
                ...
                overall_dis_fake = overall_dis_fake + self.BCE_loss(score_false, GAN_false) * float(label[j][0])
            ...
            for j in range(len(x_recon)):
                ...
                overall_gan = overall_gan + self.BCE_loss(score_fake, GAN_fake) * 2e-4 * math.exp(float(label[j+1][0])) * 20
            ...
            if self.step == 0 or self.step % self.save_and_sample_every == 0:
                ...
                save_gt = torch.reshape(input.clone().transpose(0, 1), input.shape[1:-2] + (input.shape[0]*input.shape[-2], input.shape[-1]))
                ind = 0
                list_result = torch.reshape(input.clone().transpose(0, 1), input.shape[1:-2] + (input.shape[0]*input.shape[-2], input.shape[-1]))
                for t in label[:-1]:
                    ind +=1
                    t = torch.ones(input.shape[0]).to(self.device) * float(t[0])
                    print("Referencing: ", t)
                    input = self.ema_model.sample(input, t)
                    list_result = torch.cat((list_result, torch.reshape(input.clone().transpose(0, 1), input.shape[1:-2] + (input.shape[0]*input.shape[-2], input.shape[-1]))), dim=2)
                    gt = data[:,ind,:,:].to(self.device)
                    save_gt = torch.cat((save_gt, torch.reshape(gt.clone().transpose(0, 1), gt.shape[1:-2] + (gt.shape[0]*gt.shape[-2], gt.shape[-1]))), dim=2)

                utils.save_image(save_gt.add(1).mul(0.5), str(self.results_folder / f'{self.step}_GT.png'))
                #print(list_result.shape)
                utils.save_image(list_result.add(1).mul(0.5), str(self.results_folder / f'{self.step}_samples.png'))
